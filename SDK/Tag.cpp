...
IntTag::IntTag(int value) {
	...
	if (IntTagVtable == 0x0) {
		uintptr_t sigOffset = FindSignature("48 8D 05 ? ? ? ? 48 89 07 89 5F 08");
		int offset = *reinterpret_cast<int*>(sigOffset + 3);
...
ShortTag::ShortTag(short value) {
	...
	if (ShortTagVtable == 0x0) {
		uintptr_t sigOffset = FindSignature("48 8D 05 ? ? ? ? 48 89 01 41 0F B7 40 ?");
		int offset = *reinterpret_cast<int*>(sigOffset + 3);
...
void ListTag::add(std::unique_ptr<Tag> tag) {
	...
	static ListTag__add_t func = reinterpret_cast<ListTag__add_t>(FindSignature("E8 ? ? ? ? 90 4D 8B 2E"));
